...
Le dépôt embarque un pipeline GitHub Actions générique (`.github/workflows/ci.yml`) identique à celui de l'autre application : il détecte automatiquement le type de projet (Spring Boot / Gradle ou Angular / npm) et se compose de trois jobs.
...
- **test** : exécute `run-tests.sh`, met en cache les dépendances (Gradle / npm) et publie le rapport JUnit dans le workflow.
- **build** : construit l'image Docker et la pousse sur le GitHub Container Registry (`ghcr.io`), taguée `<branche>-<sha-court>`.
- **release** : sur `main` uniquement, exécute `semantic-release` pour générer la version, le changelog et la release GitHub, synchroniser la version dans les fichiers du projet et publier l'image taguée avec la version sémantique (`X.Y.Z`).
...
Exemple : `feat(workshop): ajoute la recherche par date`
